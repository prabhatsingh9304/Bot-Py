...
def p_songs():
    ...
    try:
        ...
        play.click()
        time.sleep(10)
        blue_speak("Do you want to Continue This song? type yes/no")
        decide1=input()
        if 'no' in decide1.lower():
            driver.quit()
        else:
            time.sleep(10)
            blue_speak("Type 'stop' to stop the songs or 'p' to change song")
            decide2=input()
            if decide2=='p':
                p_songs()
            elif decide2=='stop':
...
def weather():
    ...
    tts= gTTS(text="Todays' maximum temperature is %d and minimum temperature is %d degree celsius and humidity is %d"%(maxt,mint,hum), lang='en')
    r =random.randint(1,1000000)
    audio_file= 'audio-' + str(r) + '.mp3'
    tts.save(audio_file)
    print("Maximum Temperature- %d °C\nMinimum Temperature- %d °C\nHumidity- %d"%(maxt,mint,hum))
